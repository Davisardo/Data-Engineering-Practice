import urllib.request  # buat download file dari URL
import sqlite3          # modul database bawaan Python

url_sql = "https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-DB0110EN-SkillsNetwork/Datasetteoptionallabs/Week2/data/BookShop-CREATE-INSERT.sql"
urllib.request.urlretrieve(url_sql, "BookShop-CREATE-INSERT.sql")  # download script awal (drop+create+insert BookShop)
print("Script SQL berhasil didownload!")

conn = sqlite3.connect("bookshop_normalization.db")  # database baru, khusus lab ini
with open("BookShop-CREATE-INSERT.sql", "r") as f:
    conn.executescript(f.read())  # jalankan drop+create+insert
conn.commit()
conn.close()
print("Tabel BookShop awal (belum dinormalisasi) berhasil dibuat!")

conn = sqlite3.connect("bookshop_normalization.db")  # buka lagi koneksi ke database

query_2nf = """
CREATE TABLE BookShop_AuthorDetails
(AUTHOR_ID INTEGER NOT NULL, AUTHOR_NAME VARCHAR(30) NOT NULL,
 AUTHOR_BIO VARCHAR(250), PRIMARY KEY (AUTHOR_ID));

INSERT INTO BookShop_AuthorDetails
SELECT DISTINCT AUTHOR_ID, AUTHOR_NAME, AUTHOR_BIO FROM BookShop;
"""
conn.executescript(query_2nf)  # jalankan CREATE + INSERT sekaligus
conn.commit()

# cek hasilnya
cursor = conn.execute("SELECT * FROM BookShop_AuthorDetails")
for row in cursor.fetchall():
    print(row)

conn.close()

conn = sqlite3.connect("bookshop_normalization.db")

query_pk = """
DROP TABLE IF EXISTS BookShop;

CREATE TABLE BookShop (
    BOOK_ID VARCHAR(4) NOT NULL,
    TITLE VARCHAR(100) NOT NULL,
    AUTHOR_NAME VARCHAR(30) NOT NULL,
    AUTHOR_BIO VARCHAR(250),
    AUTHOR_ID INTEGER NOT NULL,
    PUBLICATION_DATE DATE NOT NULL,
    PRICE_USD DECIMAL(6,2) CHECK(Price_USD>0) NOT NULL,
    PRIMARY KEY (BOOK_ID)
);

INSERT INTO BookShop VALUES
('B101', 'Introduction to Algorithms', 'Thomas H. Cormen', 'Thomas H. Cormen is the co-author of Introduction to Algorithms, along with Charles Leiserson, Ron Rivest, and Cliff Stein. He is a Full Professor of computer science at Dartmouth College and currently Chair of the Dartmouth College Writing Program.', 123 , '2001-09-01', 125),
('B201', 'Structure and Interpretation of Computer Programs', 'Harold Abelson', 'Harold Abelson, Ph.D., is Class of 1922 Professor of Computer Science and Engineering in the Department of Electrical Engineering and Computer Science at MIT and a fellow of the IEEE.', 456, '1996-07-25', 65.5),
('B301', 'Deep Learning', 'Ian Goodfellow', 'Ian J. Goodfellow is a researcher working in machine learning, currently employed at Apple Inc. as its director of machine learning in the Special Projects Group. He was previously employed as a research scientist at Google Brain.', 369, '2016-11-01', 82.7),
('B401', 'Algorithms Unlocked', 'Thomas H. Cormen', 'Thomas H. Cormen is the co-author of Introduction to Algorithms, along with Charles Leiserson, Ron Rivest, and Cliff Stein. He is a Full Professor of computer science at Dartmouth College and currently Chair of the Dartmouth College Writing Program.', 123, '2013-05-15', 36.5),
('B501', 'Machine Learning: A Probabilistic Perspective', 'Kevin P. Murphy', '', 157, '2012-08-24', 46);

DROP TABLE IF EXISTS BookShop_AuthorDetails;

CREATE TABLE BookShop_AuthorDetails(
    AUTHOR_ID INTEGER NOT NULL,
    AUTHOR_NAME VARCHAR(30) NOT NULL,
    AUTHOR_BIO VARCHAR(250),
    PRIMARY KEY (AUTHOR_ID)
);

INSERT INTO BookShop_AuthorDetails
SELECT DISTINCT AUTHOR_ID, AUTHOR_NAME, AUTHOR_BIO FROM BookShop;
"""
conn.executescript(query_pk)
conn.commit()
print("Primary Key berhasil diset untuk kedua tabel!")

# Test: coba insert data duplikat BOOK_ID B101, harus gagal karena PK violation
try:
    conn.execute("""
        INSERT INTO BookShop VALUES
        ('B101', 'Introduction to Algorithms', 'Thomas H. Cormen', 'duplikat test', 123 , '2001-09-01', 125)
    """)
    conn.commit()
except sqlite3.IntegrityError as e:
    print("Berhasil ditolak karena PK violation:", e)

conn.close()

conn = sqlite3.connect("bookshop_normalization.db")

query_fk = """
DROP TABLE IF EXISTS BookShop;

CREATE TABLE BookShop (
    BOOK_ID VARCHAR(4) NOT NULL,
    TITLE VARCHAR(100) NOT NULL,
    AUTHOR_NAME VARCHAR(30) NOT NULL,
    AUTHOR_BIO VARCHAR(250),
    AUTHOR_ID INTEGER NOT NULL,
    PUBLICATION_DATE DATE NOT NULL,
    PRICE_USD DECIMAL(6,2) CHECK(Price_USD>0) NOT NULL,
    PRIMARY KEY (BOOK_ID),
    FOREIGN KEY (AUTHOR_ID)
    REFERENCES BookShop_AuthorDetails(AUTHOR_ID)
    ON UPDATE NO ACTION
    ON DELETE NO ACTION
);

INSERT INTO BookShop VALUES
('B101', 'Introduction to Algorithms', 'Thomas H. Cormen', 'Thomas H. Cormen is the co-author of Introduction to Algorithms, along with Charles Leiserson, Ron Rivest, and Cliff Stein. He is a Full Professor of computer science at Dartmouth College and currently Chair of the Dartmouth College Writing Program.', 123 , '2001-09-01', 125),
('B201', 'Structure and Interpretation of Computer Programs', 'Harold Abelson', 'Harold Abelson, Ph.D., is Class of 1922 Professor of Computer Science and Engineering in the Department of Electrical Engineering and Computer Science at MIT and a fellow of the IEEE.', 456, '1996-07-25', 65.5),
('B301', 'Deep Learning', 'Ian Goodfellow', 'Ian J. Goodfellow is a researcher working in machine learning, currently employed at Apple Inc. as its director of machine learning in the Special Projects Group. He was previously employed as a research scientist at Google Brain.', 369, '2016-11-01', 82.7),
('B401', 'Algorithms Unlocked', 'Thomas H. Cormen', 'Thomas H. Cormen is the co-author of Introduction to Algorithms, along with Charles Leiserson, Ron Rivest, and Cliff Stein. He is a Full Professor of computer science at Dartmouth College and currently Chair of the Dartmouth College Writing Program.', 123, '2013-05-15', 36.5),
('B501', 'Machine Learning: A Probabilistic Perspective', 'Kevin P. Murphy', '', 157, '2012-08-24', 46);
"""
conn.executescript(query_fk)
conn.commit()
print("Foreign Key berhasil ditambahkan ke tabel BookShop!")

# Aktifkan enforcement foreign key (SQLite defaultnya OFF)
conn.execute("PRAGMA foreign_keys = ON")

# Test: coba insert AUTHOR_ID yang tidak ada di BookShop_AuthorDetails, harus gagal
try:
    conn.execute("""
        INSERT INTO BookShop VALUES
        ('B601', 'Fake Book', 'Unknown Author', '', 999, '2020-01-01', 20)
    """)
    conn.commit()
except sqlite3.IntegrityError as e:
    print("Berhasil ditolak karena FK violation:", e)

conn.close()